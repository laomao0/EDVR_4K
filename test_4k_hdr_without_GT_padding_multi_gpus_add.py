...
def main():
    ...
    subfolder_r = os.listdir(args.reference_path)
    ...
    subfolder_length = len(subfolder_l)

    subfolder_l = subfolder_l[int(part_id * subfolder_length / n_gpus) : int((part_id + 1) * subfolder_length / n_gpus)]
    logger.info(str(subfolder_r))

    for subfolder in subfolder_l:
        logger.info('{}'.format(subfolder.split('/')[-1]))
        if (subfolder.split('/')[-1] in subfolder_r):
            logger.info('{} is already exist'.format(subfolder))
            continue
