...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        ...
        util.setup_logger('base_val', opt['path']['val_log'], 'val_' + opt['name'], level=logging.INFO,
                          screen=True, tofile=True)
        ...
        for train_idx, train_data in enumerate(train_loader):
            ...
            if 'debug' in opt['name']:
                ...
                for i in range(5):
                    ...
                    save_img_path = os.path.join(img_dir,'{:4d}_{:s}_{:1d}.png'.format(train_idx, 'debug_LQ', i))
                    if opt['datasets']['train']['color'] == 'YUV':
            ...
            if (opt['train']['pixel_criterion'] == 'cb+ssim'):
                ...
                message_train_loss += ' psnr_avg : {:.2f}'.format(Avg_train_psnr.avg)
            else:
                message_train_loss = ' train_avg_loss: {:.4e}'.format(Avg_train_loss.avg)
                if opt['datasets']['train']['color'] == 'YUV':
                    message_train_loss += ' yuv_psnr_inst : {:.2f}'.format(yuv_psnr)
                message_train_loss += ' psnr_inst : {:.2f}'.format(psnr)
                if opt['datasets']['train']['color'] == 'YUV':
                    message_train_loss += ' yuv_psnr_avg : {:.2f}'.format(Avg_train_yuv_psnr.avg)
                message_train_loss += ' psnr_avg : {:.2f}'.format(Avg_train_psnr.avg)
            ...
            if current_step % opt['logger']['print_freq'] == 0:
                ...
                # tensorboard logger - avg part
                    if opt['use_tb_logger'] and 'debug' not in opt['name']:
                        if rank <= 0:
                            tb_logger.add_scalar('train_avg_loss', Avg_train_loss.avg, current_step)
                            if opt['datasets']['train']['color'] == 'YUV':
                                tb_logger.add_scalar('yuv_psnr_avg', Avg_train_yuv_psnr.avg, current_step)
                            tb_logger.add_scalar('psnr_avg', Avg_train_psnr.avg, current_step)
            ...
            if opt['model'] in ['sr', 'srgan'] and rank <= 0:  # image restoration validation
                ...
                if opt['dist']:
                    ...
                    for k, v in psnr_rlt.items():
                        ...
                        psnr_total_avg += psnr_rlt_avg[k]
                    psnr_total_avg /= max(len(psnr_rlt), 1)
                    log_s = '# Validation # PSNR: {:.4e}:'.format(psnr_total_avg)
                    for k, v in psnr_rlt_avg.items():
                        log_s += ' {}: {:.4e}'.format(k, v)
                    logger.info(log_s)
                    ...
                    ssim_total_avg /= max(len(ssim_rlt), 1)
                    ...
                    val_loss_total_avg /= max(len(val_loss_rlt), 1)
