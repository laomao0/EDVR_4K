...
def self_ensamble_pictures_create(image):
  image_rot90 = image.rot90(1, [3,4])
  image_rot180 = image.rot90(2, [3,4])
  image_rot270 = image.rot90(3, [3,4])

  image_flip = image.flip([3])
  image_flip_rot90 = image_flip.rot90(1, [3,4])
  image_flip_rot180 = image_flip.rot90(2, [3,4])
  image_flip_rot270 = image_flip.rot90(3, [3,4])

  image_all = [image, image_rot90, image_rot180, image_rot270, image_flip, image_flip_rot90, image_flip_rot180, image_flip_rot270]

  return image_all

def self_ensamble_fusion(images_list):
  images_final = images_list[0].float()
  images_final += images_list[1].rot90(3, [2,3]).float()
  images_final += images_list[2].rot90(2, [2,3]).float()
  images_final += images_list[3].rot90(1, [2,3]).float()
  images_final += images_list[4].flip([2]).float()
  images_final += images_list[5].rot90(3, [2,3]).flip([2]).float()
  images_final += images_list[6].rot90(2, [2,3]).flip([2]).float()
  images_final += images_list[7].rot90(1, [2,3]).flip([2]).float()

  images_final = images_final / 8.0
...
def main():
    ...
    frame_notation = None
    start_index = args.file_idx * 5
    end_index = args.file_idx * 5 + 5
    subfolder_l = subfolder_l[start_index : end_index]
    ...
    for subfolder in subfolder_l:
        ...
        for img_idx, img_path in enumerate(img_path_l):
            ...
            select_idx, log1, log2, nota = data_util.index_generation_process_screen_change_withlog_fixbug(
                input_subfolder, frame_notation, img_idx, max_idx, N_in, padding=padding, enable=args.use_screen_notation)
            ...
            images_all = self_ensamble_pictures_create(X0)
            output_ = []
            for X0 in images_all:
                if flip_test:
                    # output = util.flipx4_forward(model, X0)
                    output_.append(util.flipx4_forward(model, X0))
                else:
                    # output = util.single_forward(model, X0)
                    output_.append(util.single_forward(model, X0))

            output = self_ensamble_fusion(output_)
            # todo remove padding
